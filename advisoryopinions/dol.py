...
def scrape_page(page_num: int) -> None:
    page_url = (
        f"https://www.dol.gov/agencies/ebsa/about-ebsa/our-activities/resource-center/advisory-opinions?page={page_num}"
    )
    ...
    for row in page.xpath(
        "//table[contains(@class,'dol-table')]/tbody/tr"
    ):
        ct += 1
        link = row.xpath(".//td[contains(@class,'views-field-field-ebsa-document-number')]/div/a[1]")[0]
        identifier = link.xpath("text()")[0]
        doc_url = link.xpath("@href")[0]

        print(identifier, doc_url)

        pubdate = row.xpath(".//td[contains(@class,'views-field-field-ebsa-document-number')]/div/text()")[0]
        print(pubdate)
        ...
        ao = AdvisoryOpinion("Department of Labor", "DOL", identifier, pubdate, recipient, doc_url, summary=summary)
